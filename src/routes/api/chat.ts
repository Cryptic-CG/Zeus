            ...
            if (pro_requests_used >= PRO_MONTHLY_REQUEST_LIMIT) {
              ...
              return new Response(
                JSON.stringify({
                  ...
                  message: `You've reached Zeus AI Pro's Fair Usage Policy limit of ${PRO_MONTHLY_REQUEST_LIMIT.toLocaleString()} requests for this billing cycle. Your limit resets on ${resetAt.toLocaleDateString()}. Need a higher limit? Reach out to support@cryptic.technology.`,
                  reset_at: resetAt.toISOString(),
                }),
                { status: 429, headers: { "Content-Type": "application/json" } },
              );
